...
import me.bmax.apatch.ui.util.saveCustomBackground
import me.bmax.apatch.ui.util.getCustomBackgroundUri
...
@Composable
fun CustomBackgroundSettings() {
    var isCustomBackgroundEnabled by rememberSaveable { 
        mutableStateOf(APApplication.sharedPreferences.getBoolean("custom_background_enabled", false))
    }
    val prefs = APApplication.sharedPreferences
    val context = LocalContext.current
    val scope = rememberCoroutineScope()
    var cardAlpha by rememberSaveable { 
        mutableFloatStateOf(prefs.getFloat("card_alpha", 0.9f))
    }
    var cardDim by rememberSaveable { 
        mutableFloatStateOf(prefs.getFloat("card_dim", 0.5f))
    }
    ...
    if (showImageEditor && selectedImageUri != null) {
        ImageEditorDialog(
            ...
            onConfirm = { transformedUri ->
                ...
                prefs.edit {
                    putBoolean("custom_background_enabled", true)
                }
    ...
    SwitchItem(
        ...
        summary = "Set a custom background image",
    ...
    // Show sliders only when background is enabled
                    ...
                    Row(
                        ...
                        Slider(
                            ...
                            onValueChange = { newValue ->
                                cardDim = newValue
                                scope.launch {
                                    prefs.edit { putFloat("card_dim", newValue) }
                                }
                            },
                            valueRange = 0f..1f,
                            steps = 19,
