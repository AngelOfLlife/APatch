# 🌐 Добавление секретов через веб-интерфейс GitHub

## 📋 Пошаговая инструкция

### 1. Откройте настройки репозитория
- Перейдите в ваш репозиторий на GitHub
- Нажмите на вкладку **Settings** (справа сверху)

### 2. Перейдите к секретам
- В левом меню найдите **Secrets and variables**
- Нажмите **Actions**

### 3. Добавьте секреты по одному

Нажимайте **New repository secret** для каждого секрета:

#### 🔑 Секрет 1: SIGNING_KEY
```
Name: SIGNING_KEY
Secret: 
```

#### 🔑 Секрет 2: ALIAS
```
Name: ALIAS
Secret: apatch
```

#### 🔑 Секрет 3: KEY_STORE_PASSWORD
```
Name: KEY_STORE_PASSWORD
Secret: apatch123
```

#### 🔑 Секрет 4: KEY_PASSWORD
```
Name: KEY_PASSWORD
Secret: apatch123
```

#### 🔑 Секрет 5: APATCH_ORIGINAL_SIGNATURE
```
Name: APATCH_ORIGINAL_SIGNATURE
Secret: 1x2twMoHvfWUODv7KkRRNKBzOfEqJwRKGzJpgaz18xk=
```

#### 🔑 Секрет 6: APATCH_REAL_SIGNATURE
```
Name: APATCH_REAL_SIGNATURE
Secret: sypblYtJUCDSbk/u67zSBUyhRj+t7n6Tm6EPuEUnku4=
```

#### 🔑 Секрет 7: APATCH_PACKAGE_NAME
```
Name: APATCH_PACKAGE_NAME
Secret: me.bmax.apatch
```

#### 🔑 Секрет 8: APATCH_APP_CLASS
```
Name: APATCH_APP_CLASS
Secret: APApplication
```

## ✅ Проверка

После добавления всех секретов у вас должно быть 8 секретов в списке:

- ✅ SIGNING_KEY
- ✅ ALIAS  
- ✅ KEY_STORE_PASSWORD
- ✅ KEY_PASSWORD
- ✅ APATCH_ORIGINAL_SIGNATURE
- ✅ APATCH_REAL_SIGNATURE
- ✅ APATCH_PACKAGE_NAME
- ✅ APATCH_APP_CLASS

## 🚀 Что дальше?

После добавления секретов вы можете:

1. **Добавить workflow файлы** в `.github/workflows/`
2. **Запустить сборку** через Actions
3. **Получить готовый APK** без зависания

## 💡 Полезные ссылки

- Ваш репозиторий: `https://github.com/USERNAME/REPOSITORY`
- Секреты: `https://github.com/USERNAME/REPOSITORY/settings/secrets/actions`
- Actions: `https://github.com/USERNAME/REPOSITORY/actions`