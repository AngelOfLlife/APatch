# Полные GitHub Secrets для APatch с ключами подписи

## Секреты подписи приложения

```
Name: SIGNING_KEY
Value: "

gh secret set ALIAS --body "apatch"
gh secret set KEY_STORE_PASSWORD --body "apatch123"
gh secret set KEY_PASSWORD --body "apatch123"
gh secret set APATCH_ORIGINAL_SIGNATURE --body "1x2twMoHvfWUODv7KkRRNKBzOfEqJwRKGzJpgaz18xk="
gh secret set APATCH_PACKAGE_NAME --body "me.bmax.apatch"
gh secret set APATCH_APP_CLASS --body "APApplication"
```

## Важные замечания

⚠️ **Проблема зависания решена:**
- Workflow автоматически отключает проверку подписи
- Приложение будет запускаться без зависания на логотипе
- Используйте только для собственных сборок

✅ **Безопасность:**
- Храните ключи только в приватных репозиториях
- Не коммитьте ключи в код
- Проверьте права доступа к секретам